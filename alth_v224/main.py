...
class AdminPlayersSelect(discord.ui.UserSelect):
    def __init__(self):
        super().__init__(
    ...
    async def callback(self, i: discord.Interaction):
        if not await _admin_guard(i):
            return
        member = self.values[0]
        await show_admin_player_profile(i, int(member.id))


class AdminPlayersView(discord.ui.View):
    """Exception volontaire : seul écran joueur utilisant un menu déroulant Discord."""
    def __init__(self):
        super().__init__(timeout=300)
        self.add_item(AdminPlayersSelect())
        back=discord.ui.Button(label="Retour catégories",emoji="↩️",style=discord.ButtonStyle.secondary,row=1)
        async def bk(i):
            if await _admin_guard(i):
                await i.response.edit_message(content=None,embed=admin_home_embed(),view=AdminPanelView())
        back.callback=bk; self.add_item(back)
